...
% --- IEEEtran .bst control-entry support (for non-IEEEtran doc classes) ---
\makeatletter
...
An AI coding assistant (Claude) was used in a limited, supporting capacity for
this project. Its use was restricted to two areas. First, transcribing and
typesetting: the model derivations, the design reasoning and the analysis in
this report were worked out by hand on paper, and Claude was used to
help convert that handwritten work into LaTeX and to tidy the document
formatting. Second, plotting: Claude helped write parts of the Python
scripts used to create the figures from the result data. It
was not used to derive the model,
implement the core \texttt{emdw} inference or EM code, or interpret the results.
Claude was also given the full emdw library codebase and wrote a reference manual
that I used often while writing the implementation, as it contained good emdw and C++
practices and explanations. All derivations, modelling and design decisions, code implementation, 
and the analysis of all results are my own work. I also used claude to setup building so that I could run
my code outside of /devel/emdw and have it work and build in a separate folder. 
...
I formulated the wumpus tracking problem as a hidden Markov model, using a
uniform random-walk transition between cells and a per-cell Bernoulli
detection model (Section~\ref{sec:design}). This structure was chosen because
the wumpus moves according to Markov dynamics, and because the per-cell
detections are conditionally independent given the wumpus position. I derived
the model by hand, with guidance from Barber's work on hidden Markov
models~\cite[Ch.~23]{barber2012bayesian}. The main simplification, in which
the $(T{+}1)G$ binary detection variables are collapsed into $T{+}1$ unary
evidence factors on the wumpus positions, is derived in
Section~\ref{sec:obs}. The main design trade-offs, specifically learning
$p_w$ and $p_c$ as point estimates by EM rather than by full Bayesian inference,
and restricting the work to
datasets 1--3, are discussed in Sections~\ref{sec:scope}
and~\ref{sec:results}.

\paragraph{GA\,2 - Application of scientific and engineering knowledge.}
The solution applies probability theory (joint factorisation, marginalisation
and conditional independence), graphical-model theory (Bayes networks, factor
graphs and cluster graphs) and the relevant inference algorithms (sum-product
belief propagation and the Expectation--Maximisation algorithm) to the
engineering problem of tracking a target from noisy measurements. The C++
implementation, built on the \texttt{emdw} library, realises this design
(Section~\ref{sec:impl}). The convergence behaviour of the EM parameter learning is
...
The conditional independence assumptions encoded by the model are (from [\cite{barber2012bayesian}, Ch.~23]) the following:
\begin{itemize}
...
Due to a lack of information about the wumpus's starting cell, a uniform
prior is used:
\begin{equation}
...
\llbracket \,\Move(i, d) = j\,\rrbracket
\]
...
Where $\circled{1}{-0.5pt}{0.7}$ is expanded to the following to remove terms independent of $x^\star$:
\[
...
The factor list is passed to \texttt{emdw}'s \texttt{ClusterGraph}
constructor with either the \texttt{LTRIP}, \texttt{BETHE} or \texttt{JTREE} construction
tag. For the chain-structured factor graph in this model, a non-loopy
...
where the wumpus is would also probably need to know how likely
...
The M-step is a Bernoulli MLE similar in structure to the
Gaussian-mixture derivation in Week~10 of the class
notes~\cite[Week~10]{dupreez2026de424}. With
$A_t = \sum_x \gamma_t^{(k)}(x)\, d_t^{(x)}$ (the expected
detection at the wumpus cell at time $t$) and
$D_t = \sum_i d_t^{(i)}$ (the total detection count at time $t$),
...
values.EM is initialised at $(p_w^{(0)}, p_c^{(0)}) = (0.9, 0.1)$.
...
The implementation consists of two main programs and a shared helper library:
\begin{itemize}
...
output results, but with the EM variant wrapping factor build, CG
...
(\texttt{LTRIP}, \texttt{JTREE} or \texttt{BETHE}) on the command line. The code was
written with close reference to \cite{emdw_userguide}, and with reference to an LLM generated user guide 
that had full context from the emdw library files and guided me on proper C++ syntax and usage for emdw-specific
cases.
...
a single length-$G$ cell domain created once:
...
\item \textbf{Mean cell distance}: average Euclidean distance
...
\end{itemize}
The evaluation uses posterior heatmaps for representative time
...
over 25 iterations, the mean time for \texttt{LTRIP} was lowest at 0.5s, with \texttt{JTREE} at 0.55s and \texttt{BETHE} at
0.65s, all with negligible difference in time variance. In a situation where inference about the current position of the wumpus
...
faster inference of the wumpus would be more accurate, because the wumpus is running away (and thus changing its position) as its position is being
inferred. 
...
it was incorrect it was incorrect but unsure of its correctness. The EM algorithm was able to learn the parameters
for dataset 3, however the accuracy was worse than datasets 1 and 2, which is a result that would require further investigation to fix.
The cluster graph choice did not have a significant effect on the results, however \texttt{LTRIP} was slightly faster than
\texttt{JTREE} and \texttt{BETHE}. An interesting observation is that the accuracy of inference using miniproject.cc with dataset 4, 
and miniproject\_q3.cc with dataset 5 resulted in accuracies of 89\% and 84.5\% respectively. This, while not achieving the occupied cell
learning, shows that in a real world situation where the wumpus is being tracked, learning the occupied cells would be more accurate, but using
these simpler models might be a worthwhile trade-off in the time-sensitive application.
